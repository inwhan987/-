...
def score_sentiment_llm(text: str, max_retries: int = 5, symbol: str | None = None) -> SentimentResult | None:
    ...
    _neg_rule = (
        ...
        "실적 관련 문맥 판단 — 반드시 적용:\n"
        "  실적/매출/영업이익 + (감소·하락·부진·둔화·줄었·줄어·역성장) → 음수\n"
        "  실적/매출/영업이익 + (전년대비·전분기대비·YoY·QoQ) + (감소·하락) → 음수\n"
        "  예상·컨센서스·시장전망 + (하회·미달·못미쳐) → 음수\n"
        "  단순 '실적발표·실적공시' 만으로는 호재 판단 금지 → 중립(0)\n"
        "  실적 + (증가·성장·개선·상회·초과·서프라이즈) → 양수\n"
    )
    if company_name:
        prompt = (
            f"너는 한국 주식 투자 전문가다.\n\n"
...
def score_sentiment_llm_batch(
    ...
    if company_name:
        prompt = (
            f"너는 한국 주식 투자 전문가다. 각 헤드라인이 주가에 미치는 영향을 평가해라.\n\n"
            f"종목: {company_name}\n\n"
            f"점수 기준 (-1.0 ~ +1.0):\n"
            f"  +0.7~+1.0: 강한 호재 (실적 급증·수주 대박·신약 승인·배당 확대 등)\n"
            f"  +0.3~+0.7: 호재 (실적 개선·신제품 출시·계약 체결 등)\n"
            f"  -0.3~+0.3: 중립 (단순 인사·업황 소개·IR 행사 등)\n"
            f"  -0.3~-0.7: 악재 (실적 부진·소송·규제·경쟁 심화 등)\n"
            f"  -0.7~-1.0: 강한 악재 (파업·파산·횡령·사고·리콜·상장폐지·적자전환 등)\n\n"
            f"악재 판단 원칙 — 아래 키워드 포함 시 반드시 음수:\n"
            f"  파업, 총파업, 노조, 쟁의, 소송, 기소, 구속, 횡령, 배임, 과징금,\n"
            f"  적자, 손실, 하락, 리콜, 사고, 화재, 폭발, 유출, 해킹,\n"
            f"  감자, 유상증자, 거래정지, 상장폐지, 경영권분쟁, 파산\n\n"
            f"실적 문맥 판단 — 반드시 적용:\n"
            f"  실적/매출/영업이익 + (감소·하락·부진·둔화·줄었·역성장) → 음수\n"
            f"  전년대비·전분기대비·YoY·QoQ + (감소·하락) → 음수\n"
            f"  예상·컨센서스 + (하회·미달·못미쳐) → 음수\n"
            f"  단순 '실적발표·공시' 만 → 중립(0.0)\n"
            f"  실적 + (증가·성장·개선·상회·서프라이즈) → 양수\n\n"
            f"관련도 기준:\n"
            f"  A = {company_name} 직접(실적·제품·수주·주가·지배구조·공장·노사)\n"
            f"  B = 동일섹터·경쟁사·코스피시황\n"
            f"  C = 타업종·무관(회사명만 언급·CSR·부동산)\n\n"
            f"출력: JSON 배열만. 형식: [[점수,관련도],...]\n"
            f"예시: [[0.8,\"A\"],[-0.7,\"A\"],[0.1,\"B\"],[0.0,\"C\"]]\n"
            f"설명·번호·줄바꿈 금지.\n\n"
            f"헤드라인:\n{numbered}"
        )
    else:
        prompt = (
            f"너는 한국 주식 투자 전문가다. 각 헤드라인이 주가에 미치는 영향을 평가해라.\n\n"
            f"점수 기준 (-1.0 ~ +1.0):\n"
            f"  +0.7~+1.0: 강한 호재 (실적 급증·수주·신약 승인 등)\n"
            f"  +0.3~+0.7: 호재\n"
            f"  -0.3~+0.3: 중립\n"
            f"  -0.3~-0.7: 악재 (실적 부진·소송·규제 등)\n"
            f"  -0.7~-1.0: 강한 악재 (파업·파산·횡령·사고·적자전환 등)\n\n"
            f"악재 판단 원칙 — 아래 키워드 포함 시 반드시 음수:\n"
            f"  파업, 소송, 기소, 구속, 횡령, 배임, 과징금, 적자, 손실,\n"
            f"  리콜, 사고, 화재, 유출, 해킹, 감자, 유상증자, 파산\n\n"
            f"실적 문맥 판단 — 반드시 적용:\n"
            f"  실적/매출/영업이익 + (감소·하락·부진·둔화·줄었·역성장) → 음수\n"
            f"  전년대비·전분기대비·YoY·QoQ + (감소·하락) → 음수\n"
            f"  예상·컨센서스 + (하회·미달) → 음수\n"
