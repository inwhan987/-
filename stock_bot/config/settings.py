...
class Settings(BaseSettings):
    ...
    # 값(2026-09-19, 백테스트 9,095건 전략별 그리드, train 23-01~24-06 / test 24-07~25-08 양쪽에서 공통 규칙보다
    # 나은 조합만 채택). VALUE_PURE 는 백테스트 표본 0 → 공통 규칙. 실거래 MFE/MAE(positions.mfe_pct/mae_pct) 쌓이면 재조정.
    swing_exit_by_strategy: str = Field(default=(
        "NEWHIGH:stop=0.07,tp=0,trail=0,ma=20;MOMENTUM:tp=0,trail=0,ma=5;"
        "BREAKOUT:tp=0,trail=0,ma=10;GAPGO:tp=0.08,trail=0,time=10;VALUE_MOM:stop=0.10,tp=0.08,trail=0,ma=5;"
        "PULLBACK:stop=0.15,tp=0,trail=0;FLOW_PULLBACK:tp=0.08,trail=0;MEANREV:tp=0.08,trail=0;"
        "FLOW_FORGN:tp=0.15,trail=0;FLOW_INST:tp=0.08,trail=0,time=10;FLOW_BOTH:tp=0,trail=0"))
    ...
    leader_budget_krw: float = Field(default=1_000_000)   # 대장주 총 예산(원). leader_slot_budget_krw 미설정(0) 시 슬롯 수로 나눠 1건당 예산 산정.
    leader_max_positions: int = Field(default=1)          # 하루 동시진입 가능 종목 수(=슬롯). 1(기본)=기존 "하루 1종목 1회". >1: 보유 종목은 그대로 두고 남은 슬롯으로 다른 섹터 진입 계속.
    leader_slot_budget_krw: float = Field(default=0)      # 1회 진입 시 투입금액(원) 직접 지정. 0(기본)=미설정 → leader_budget_krw/leader_max_positions 로 자동 산정(기존 방식과 동일). >0 이면 이 값을 그대로 슬롯예산으로 사용(슬롯 수와 무관, 총예산 상한 체크 없음).
    ...
    # 2026-08-28 폐기: 호가 스윕 지정가(leader_sweep_*) 대신 시장가 + 부족분
    # 재주문으로 되돌렸다. 지정가는 미체결 잔량이 호가창에 남아 '확정 조회 →
    # 취소' 사이에 체결이 더 붙고, 그 차이가 감시 없는 유령 잔량이 된다
    # (003350: 30주로 확정했는데 실제 61주 체결 → 31주 방치).
    # 시장가는 잔량이 즉시 자동취소돼 체결수량이 조회 시점과 무관하게 확정된다.
    ...
    #        이후 고점(peak) 대비 leader_trail_gap_pct% 밀리면 그때 전량 청산
    #        (stop 은 고점 추종으로만 올라가고 절대 내려가지 않음).
    # split: 2단계 분할익절. leader_split_tp1_pct 도달 시 물량의
    #        leader_split_tp1_ratio% 를 먼저 익절하고, 손절선을 그 가격으로
    #        올려 잔량 확정수익을 잠근다(본전확보). 이후 leader_split_tp2_pct
    #        도달 시 잔량 전량 청산, 그 전에 손절선(=1차 목표가) 밀리면 잔량도
    #        그 가격에서 청산.
    leader_exit_mode: str = Field(default="fixed")
    leader_trail_activate_pct: float = Field(default=4.0)  # [trail] 발동 수익률 %(기본 leader_tp_pct와 동일)
    leader_trail_gap_pct: float = Field(default=1.5)       # [trail] 발동 후 고점 대비 추종 갭 %
    leader_split_tp1_pct: float = Field(default=2.0)       # [split] 1차 익절 수익률 %
    leader_split_tp1_ratio: float = Field(default=50.0)    # [split] 1차 익절 물량 비율 %(반올림)
    leader_split_tp2_pct: float = Field(default=4.0)       # [split] 2차(잔량) 익절 수익률 %
    ...
    leader_vwap_max_pull_pct: float = Field(default=2.0)   # vwap_touch 전용 붕괴컷: 확정봉 종가가 당일 전고점 대비 -N% 보다 깊게 밀린 상태면 진입 차단 (0=끔). 2026-08-18 스윕(5종목×7거래일, 동시1종목 제약): 컷없음 16건 승률37.5%/누적-5.4% → -2% 13건 61.5%/+6.6%. 깊은 눌림은 눌림목이 아니라 추세붕괴라 반등 확률이 역전됨. or_mode 의 VWAP 분기에도 동일 적용(컷에 걸리면 스윙저점 폴백으로 넘어감).
    leader_vwap_min_slope_pct: float = Field(default=0.0)   # vwap_touch 전용 기울기컷: 최근 5확정봉(3분봉=15분) VWAP 상승률이 +N% 미만이면 진입 차단 (0=끔, 기본). VWAP 은 9시부터의 누적 거래량가중 평균이라 관성이 커서, 이게 올랐다는 건 신규 물량이 당일 평균가 위에서 체결됐다는 뜻(실매집). 2026-08-18 스윕: ≥+0.2% 승률 72.7%(n=11)·붕괴컷과 조합 시 85.7%(n=7) — 표본이 얇아 채택 보류, 기본 0 으로 두고 vwap_probe 로그만 적재해 사후 검증한다.
    ...
    leader_mf_clamp_high:   float = Field(default=1.5)       # 배수 상한 (활황기 방어) — 2026-08-11 2.0→1.5 (스케일 매칭 후에도 활황 상한 완화)
    ...
    leader_review_llm_min_signals: int = Field(default=8)   # LLM 해석 최소 당일 신호판정 건수(미진입·보류·신호스킵 합). 체결 0건이어도 깔때기에 이만큼 쌓였으면 '왜 안 샀나'를 해석할 표본은 충분하다. 0=끔(체결 기준만 사용)
