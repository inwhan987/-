...
class Settings(BaseSettings):
    ...
    leader_entry_mode: str = Field(default="vwap_touch")  # 진입 방식(vwap 고정, 2026-08-03 승인): pullback=구현행(스윙저점+RECLAIM, 전고점 floor) · vwap_touch=교과서 VWAP 첫눌림(직전봉 VWAP위→저가 VWAP터치→종가 되받음). vwap_touch면 w·max_pull·phwin·fib·anchor·anchor_ema·reclaim·volfilter 무시(스윙/floor/앵커 미사용), stop_buf(참조=터치봉저가)·tp·bar_range·close_time 동일. 4일표본 백테 +35% vs pullback +0.43%(표본부족·폭주주누락 캐비엇, 관전 검증중)
