    ...
    def _leader_pick_tick():
        ...
        try:
            ...
            if picks.exists():
                ...
                # 2026-08-26: 선별 성공 즉시 매매 tick 을 한 번 당겨 돌린다.
                # 예전엔 다음 분 크론(:00)을 기다려서 바스켓 적재가 최대 60초
                # 늦었다(08-26: 선별 09:30:54 → 적재 09:31:00). 진입 판정은
                # 3분 확정봉 기준이라 타점 이득은 크지 않지만, 바스켓·대시보드가
                # 비어 보이는 구간이 사라진다. 선별 성공은 하루 1회뿐이고
