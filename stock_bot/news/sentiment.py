...
def score_sentiment_llm(text: str, max_retries: int = 5, symbol: str | None = None) -> SentimentResult | None:
    ...
    if company_name:
        prompt = (
            f"주식뉴스감성분석. 종목:{company_name} 헤드라인:{text}\n"
            f"점수-1~+1: 강호재+0.7↑ 호재+0.3↑ 중립±0.3 악재-0.3↓ 강악재-0.7↓\n"
            f"음수강제: 파업·노조·쟁의·소송·기소·구속·횡령·배임·과징금·적자·손실·리콜·사고·화재·유출·해킹·감자·유상증자·상장폐지·파산\n"
            f"실적맥락: 실적/매출+감소/하락/부진/하회→음수, 단순발표→0, 증가/상회→양수\n"
            f"관련도: A={company_name}직접 B=섹터/시황 C=무관\n"
            f"출력: 점수 관련도 예)-0.8 A 설명금지"
        )
    else:
        prompt = (
            f"주식뉴스감성분석. 헤드라인:{text}\n"
            f"점수-1~+1: 강호재+0.7↑ 호재+0.3↑ 중립±0.3 악재-0.3↓ 강악재-0.7↓\n"
...
def score_sentiment_llm_batch(
    ...
    if company_name:
        prompt = (
            f"주식뉴스감성분석. 종목:{company_name}\n"
            f"점수-1~+1: 강호재+0.7↑ 호재+0.3↑ 중립±0.3 악재-0.3↓ 강악재-0.7↓\n"
            f"음수강제(해당키워드포함시): 파업·노조·쟁의·소송·기소·구속·횡령·배임·과징금·적자·손실·리콜·사고·화재·유출·해킹·감자·유상증자·상장폐지·파산\n"
            f"실적맥락: 실적/매출/영업이익+감소/하락/부진/하회→음수, 단순실적발표→0, 실적+증가/상회/서프라이즈→양수\n"
