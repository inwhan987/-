...
def compute_levels(code: str, daily: pd.DataFrame, c: SwingCfg) -> dict:
    ...
    last = d.iloc[-1]
    return _levels(last, float(d["high"].tail(20).max()), c)


def from_scan_row(r: pd.Series, c: SwingCfg) -> dict:
    """daily_scan.scan_panel 행(이미 지표값이 있음) → 기준선."""
    return _levels(r, float(r["box_top"]), c)


def _levels(row, box_top: float, c: SwingCfg) -> dict:
    ...
    return {
        ...
        "stop_px": round(close * (1 - c.stop_pct), 2),
        "tp_px": round(close * (1 + c.tp_pct), 2),
        "ref_value_ma20": vma,
        "ref_mktcap": cap,
    }


def entry_levels(entry_px: float, c: SwingCfg) -> tuple[float, float]:
    """체결가 기준 손절·익절가."""
    return entry_px * (1 - c.stop_pct), entry_px * (1 + c.tp_pct)
